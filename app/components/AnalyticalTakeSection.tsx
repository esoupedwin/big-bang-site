...
const CACHE_KEY = (k: string) => `analytical-take:${k}`;

export function AnalyticalTakeSection({ topicKey, label, content, diff }: Props) {
  const [text,    setText]    = useState(() =>
    typeof window !== "undefined" ? (sessionStorage.getItem(CACHE_KEY(topicKey)) ?? "") : ""
  );
  ...
  useEffect(() => {
    if (sessionStorage.getItem(CACHE_KEY(topicKey))) return;
    ...
    fetch("/api/coverage/analytical-take", {
      ...
      .then(async (res) => {
        if (!res.ok || !res.body) { setError("Failed to generate analytical take."); setLoading(false); return; }
        ...
        if (!signal.cancelled && accumulated) {
          sessionStorage.setItem(CACHE_KEY(topicKey), accumulated);
        }
        setLoading(false);
      })
      .catch(() => { if (!signal.cancelled) { setError("Failed to generate analytical take."); setLoading(false); } });

    return () => { signal.cancelled = true; };
  }, [topicKey, label, content, diff, retry]);
    ...
    <div className="p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900">
      ...
      {error && (
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => { sessionStorage.removeItem(CACHE_KEY(topicKey)); setError(""); setText(""); setRetry(r => r + 1); }}
